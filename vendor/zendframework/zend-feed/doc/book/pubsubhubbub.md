...
to enable posts (Publishers) to "push" updates from their RSS or Atom feeds
(Topics) to end Subscribers. These Subscribers will have subscribed to the
post's RSS or Atom feed via a Hub, a central server which is notified of any
...
traditional model where post feeds have been polled at regular intervals to
...
updates. This may be a post, an aggregator, or even a web service with a public
